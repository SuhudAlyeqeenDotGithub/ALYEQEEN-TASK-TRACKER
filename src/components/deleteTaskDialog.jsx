...
const DeleteTaskDialog = ({tasksToDelete}) => {
    ...
    const {deleteTaskDialogIsOpen, setDeleteTaskDialogIsOpen, deleteTaskFromView, setDeleteTaskFromView} = useContext(TaskDialogContext);
         ...
         deleteTaskDialogIsOpen && (
              <>
                ...
                <AllPurposeContainer containerStyling={dialogueStyling}>
                  <RegularParagraph styling="text-blue-900 font-bold text-xl w-full text-center mb-5">Head Ups !</RegularParagraph>
                  <RegularParagraph styling="text-md text-blue-900 font-semibold">Are you sure you want to delete the below task(s)?</RegularParagraph>

                  <div className={`${scrollBarStyling} w-full ml-5 mr-5 border border-blue-900 p-2 flex flex-wrap rounded-md mt-5 max-h-[200px]`}>
                  <RegularParagraph>{taskLengthToDisplay}</RegularParagraph>


                   <div className="w-full">
                   {seeLessIsClicked ? (<button className={seeMoreSeeLessStyling} onClick={handleSeeMore}> See More...</button>):
                   (<button className={seeMoreSeeLessStyling} onClick={handleSeeLess}> See Less...</button>)}
                   </div>
                  ...
                  <div className="w-full flex flex-row justify-center gap-x-10 mt-5">
                    <button onClick={deleteTasks} className={`hover:text-white hover:bg-red-500 hover:border-none ${buttonStyling}`}>Delete</button>
                    <button onClick={cancelDelete} className={`hover:text-white hover:bg-blue-900 hover:border-none ${buttonStyling}`}>Cancel</button>
                  </div>
