...
export const TaskStatusChip = ({ children }) => {
  ...
  useEffect(() => {
    if (children === "Completed") {
      setChipStyling(`bg-green-200 border border-green-700 text-green-800 font-semibold shadow rounded-3xl pl-4 pr-4 pt-2 pb-2 justify-self-start`);
    } else if (children === "In Progress") {
      setChipStyling(`bg-yellow-200 border border-yellow-700 text-yellow-800 font-semibold shadow rounded-3xl pl-4 pr-4 pt-2 pb-2 justify-self-start`);
    } else {
      setChipStyling(`bg-blue-200 border border-blue-700 text-blue-800 font-semibold shadow rounded-3xl pl-4 pr-4 pt-2 pb-2 justify-self-start`);
    }
  }, [children]);
